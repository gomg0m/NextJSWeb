        ...
        const user = Axios.get('/api/getUserCookieInfo').then((res)=> {
            ...
            if (res.status == 200) {
              if (res.data.statusCode == 1) { //쿠키 있을때 처리 루프
                ...
                console.log("ParentFunc Called", data);
              } else {  //쿠키 없을떄 처리 루프
              let today = String(new Date());
          ...
          <Box sx={style}>
            ...
            <div className={classes.ContainerMain}>
                ...
                <div className={classes.ContainerSub2}>
                    <div className={classes.Title}><div>제목<span style={{color:'red'}}>*</span></div> <div style={{marginLeft:'80px', width:"600px"}} ><FormInputText name="title" control={control} label="제목을 입력해 주세요"/></div></div>
                    <div className={classes.Content}><div>내용<span style={{color:'red'}}>*</span></div> <div style={{marginLeft:'70px', width:"600px"}} ><FormInputMultilineText name="content" control={control} label="공지사항 내용을 작성해 주세요"/></div></div>
                    <div className={classes.ImgUpload}><div>첨부이미지</div><div style={{marginLeft:'-5px', width:"500px"}} > <ImgUpload /> </div> </div>
                </div>
