...
export default function Rightside(props) {
  ...
  function getCommentTable(tableID){
      Axios.post("/api/getProductComment", {tableID}).then((res)=>{
          if (res.status == 200 )
          {
            ...
            res.data.users.map((item)=>{
              if(item.prdtcomment_image){
                //// prdtcomment_image를 일반 스트링 배열로 전환
                let parsedPhotos = JSON.parse(item.prdtcomment_image);
                let photosFormat =[];
                ...
                parsedPhotos.map((photo)=>{
                  ...
                  tmp_cmt.push({
                  id:item.prdtcomment_id, 
                  name: item.prdtcomment_name,
                  team: item.prdtcomment_team,
                  lasttime: item.prdtcomment_lasttime,
                  contents: item.prdtcomment_contents,
                  image: parsedPhotos,
                })                                  
              }
              else {
  ...
  function onClickDiscussWrite (params){
    ...
    Axios.post("/api/insertProductComments", {data}).then((res)=>{
      if (res.status == 200 )
      {
        ...
        <Box sx={{ width: '100%' }}>  {/*----- Tab 메뉴 -----*/}
          ...
          <TabPanel value={tabValue} index={2}>  {/*----- 의견 Tab 내용 -----*/}              
...

              {
                ...
                comment.map((item)=>(<TechCommentElement value={item}/>))
