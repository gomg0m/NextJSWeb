...
export default function Rightside(props) {
  ...
  function getCommentTable(tableID){
      Axios.post("/api/getPostComment", {tableID}).then((res)=>{
          if (res.status == 200 )
          {
            ...
            res.data.users.map((item)=>{
              if(item.postcomment_image){
                //// postcomment_image를 일반 스트링 배열로 전환
                let parsedPhotos = JSON.parse(item.postcomment_image);
                let photosFormat =[];
                ...
                parsedPhotos.map((photo)=>{
                  ...
                  tmp_cmt.push({
                  id:item.postcomment_id, 
                  name: item.postcomment_name,
                  team: item.postcomment_team,
                  lasttime: item.postcomment_lasttime,
                  contents: item.postcomment_contents,
                  image: parsedPhotos,
                })                                  
              }
              else {
  ...
  function onClickDiscussWrite (params){
    ...
    Axios.post("/api/insertPostComments", {data}).then((res)=>{
      if (res.status == 200 )
      {
